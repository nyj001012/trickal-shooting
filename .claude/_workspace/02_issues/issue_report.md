...
| --- | --- |
| 이슈 번호 | #13 |
| URL | https://github.com/nyj001012/trickal-shooting/issues/13 |
| 제목 | `[Balance] MANA 자연 회복 속도 1/10 조정` |
| 상태 | Open |
...
| 항목 | 값 |
| --- | --- |
| 브랜치명 | `balance/13-slower-mana-regen` |
| 파생 기준 | 최신 `origin/main` (`00c62d2`) |
...
| --- | --- |
| 이슈 번호 | #15 |
| URL | https://github.com/nyj001012/trickal-shooting/issues/15 |
| 제목 | `[Gameplay] 스킬탄 관성 유도와 발사 확산` |
| 상태 | Open |
...
| 항목 | 값 |
| --- | --- |
| 브랜치명 | `feature/15-steering-homing-projectiles` |
| 파생 기준 | 최신 `origin/main` (`73abfe0`) |

## 완료 조건

- [x] 주입형 RNG로 스킬탄 초기 Y 속도를 `[-120, 120)` px/sec 범위에 분산
- [x] 스킬탄이 최초 획득한 생존 적 ID를 유지하고 대상 소멸 시에만 최근접 적 재획득
- [x] 중심 거리 `150px` 미만에서는 `0.3`, 그 외에는 `0.06` 회전 계수 적용
- [x] 락온 유지·재획득·동률·무타겟 관성·0벡터 안전·동일 시드 결정성과 `720` px/sec 속력 보존 검증
- [x] 기존 MANA·발사 주기·피해·충돌·반응형 동작 유지
- [x] lint, typecheck, unit/component test, coverage, format, build, Chromium E2E 통과
- [x] 작업 브랜치 Push 및 이슈 #15를 연결한 PR 생성·갱신
...
| --- | --- |
| PR 번호 | #16 |
| URL | https://github.com/nyj001012/trickal-shooting/pull/16 |
| 기준/작업 브랜치 | `main` ← `feature/15-steering-homing-projectiles` |
| 상태 | Open, MERGEABLE |
| CI | 등록된 원격 체크 없음(`statusCheckRollup: []`) |
