...
- **문서 버전:** 3.3 (적 좌측 이탈 시 HP 감소 규칙 제거. 이탈 적은 제거만 하며 HP 감소는 직접 접촉 피해로 한정)
...
| 발사·적 이동·충돌 미구현(TODO 주석) | 스페이스바 수동 발사 + 쿨다운, 적 좌측 직진·이탈 시 제거, 직접 접촉 피해와 **피격 무적 시간 신설** | §9 D-2/D-5 확정. 8방향 이동으로 플레이어가 적에게 돌진 가능해져 `INV-DMG-1`(무적 시간)이 필수가 됨(§6.2.1-(5)) |
