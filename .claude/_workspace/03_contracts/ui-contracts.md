...
- **근거:** design.md v3.5 §6.2.1-(1), §6.7, §6.9, §9 D-1/D-2/D-3/D-6
...
**정밀 규칙**
- `{hp}`, `{maxHp}`, `{score}`, `{level}`은 **정수를 그대로** 문자열 보간한다(소수점·천단위 콤마 없음). `{mana}`는 초당 회복·소모로 소수가 될 수 있으므로 `Math.floor`로 내림한 정수만 표시한다. 내부 MANA는 정밀도를 유지하며 HUD 발행 시에만 내린다.
...
| 스킬 발사 | `Space` | `skill` | MANA 20 이상에서 시작, 누르는 동안 유지(D-2/D-3) |
...
- 4개 방향키와 `Space`의 브라우저 기본 동작(페이지 스크롤)은 `preventDefault()`로 차단한다.
- `window`의 `blur` 이벤트에서 `InputState`의 6개 필드를 **모두 `false`로 리셋**한다(키 고착 방지).
- 일반탄은 별도 입력 없이 자동 발사된다. `Space`는 일반탄을 수동 발사하는 키가 아니라 MANA 스킬 모드 전용 입력이다. E2E는 입력 없는 일반탄 발사와 Space를 누르는 동안의 MANA 감소·해제 후 회복을 각각 검증한다.
...
- 키보드로 이동·스킬 발사·재시작이 가능해야 한다. 일반탄은 별도 조작 없이 자동으로 진행되고, 스킬탄은 Space를 누르는 동안 MANA 규칙에 따라 발사된다.
...
- 모바일 지원은 이번 단계에서 **가로 표시 레이아웃**에 한정한다. 일반탄은 자동이며 터치 이동·스킬·재시작 입력은 이 계약에 포함하지 않는다.
