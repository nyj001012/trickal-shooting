...
- **근거:** design.md v3.2 §6.2.1-(1), §6.7, §6.9, §9 D-1/D-2/D-6
...
| 발사 | `Space` | `fire` | 누르고 있으면 쿨다운마다 연사(D-2) |
...
- `Space`, 4개 방향키의 브라우저 기본 동작(페이지 스크롤)은 `preventDefault()`로 차단한다.
- `window`의 `blur` 이벤트에서 `InputState`의 6개 필드를 **모두 `false`로 리셋**한다(키 고착 방지).
- E2E(`Playwright`)는 `page.keyboard.down('Space')` / `page.keyboard.press('KeyR')` 등 **Playwright의 키 이름**(대부분 `event.code`와 동일 문자열)을 사용해 위 표와 1:1 매칭한다.
...
- 키보드만으로 전체 플레이(이동/발사/재시작)가 가능해야 하며, 이는 §2 키 바인딩 표로 완전히 커버된다.
...
- 모바일 지원은 이번 단계에서 **가로 표시 레이아웃**에 한정한다. 터치 이동·발사·재시작 입력은 이 계약에 포함하지 않는다.
