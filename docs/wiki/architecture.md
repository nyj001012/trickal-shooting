...
2. 플레이어·적·일반탄 이동과 스킬탄 락온·근접 부스트 관성 조향 및 화면 이탈 처리, 회복 젤리 수명 타이머 감소
3. 적 스폰 타이머 처리
4. 부수효과 없는 AABB 충돌 탐지
5. 일반탄·스킬탄 피해·보상을 분리 적용하고 적 투사체 피해 처리, 플레이어 접촉 피해 처리, 투사체 처치 시 10% 확률로 회복 젤리 드롭, 플레이어의 젤리 획득 처리 (issue #21)
...
**플래그 기반 단 1회 선택 (스폰 직후 즉시)**
각 적은 `actionInitialized: boolean` 플래그를 갖는다. `spawnTick`은 새 적을 `actionInitialized = false`로 초기화하고 나머지 행동 필드는 플레이스홀더로 채운다. 같은 틱에 `updateEnemyAi` 시스템이 `actionInitialized === false`인 적을 발견하면 즉시 첫 실제 행동을 무작위로 선택하고, 선택을 마친 시점에 `actionInitialized = true`로 뒤집는다. 이후 그 적에 대해서는 `updateEnemyAi`가 어떤 필드도 재선택하지 않는다. 이 구조는 기존 투사체 쿨다운과 스폰 타이머와 달리 카운트다운 개념 없이, 생애 중 단 1회 선택만 보장한다.
...
`applyMovement` 시스템은 매 틱 해당 행동의 위치 공식만 적분하고, `updateEnemyAi` 시스템(순서상 이동보다 뒤에 배치)은 최초 행동 선택만 담당한다(재선택 없음). 덕분에 같은 틱에서 읽기-쓰기 경합이 없으면서도 스폰 직후 즉시 행동이 배정되고, 이후는 절대 바뀌지 않는다.
