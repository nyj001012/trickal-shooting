...
1. 일반탄·스킬탄 쿨다운 감소, Space/MANA 모드 전이와 상호 배제 발사
2. 플레이어·적·일반탄 이동과 스킬탄 최근접 유도 및 화면 이탈 처리
3. 적 스폰 타이머 처리
...
`GameWorld`는 `regularProjectiles`와 `skillProjectiles`를 별도 배열로 보관한다. 두 탄종은 엔티티 타입, 생성 쿨다운, 이동, 충돌 결과, 전투 보상, Canvas 렌더 순회까지 독립된 경로를 사용한다. 일반탄은 +x 직진하며 처치 시 MANA를 지급하고, 스킬탄은 매 틱 최근접 생존 적을 향해 속도 벡터를 갱신하며 처치 시 SCORE만 지급한다.

플레이어는 두 쿨다운과 `isSkillFiring` 상태를 보유한다. 일반 상태에서는 0.3초 자동 발사와 초당 MANA 5 회복이 실행된다. MANA 20 이상에서 Space를 누르면 스킬 상태에 진입해 일반탄을 완전히 중지하고 0.15초 스킬 발사와 초당 MANA 30 소모만 실행한다. Space 해제 또는 MANA 고갈 후 일반 상태로 돌아온다. 모든 MANA 변경은 0~100으로 포화되어 100에서 추가 증가해도 초기화되지 않는다.
...
- 스킬탄은 MANA 규칙을 만족하는 Space 입력 중에만 생성되고 최근접 생존 적을 유도한다.
...
백엔드, 인증, 데이터베이스, 멀티플레이, 랭킹, 로컬 영속화, CI/CD, 배포 설정, 실제 이미지·사운드 에셋, 모바일 터치 조작, 적 투사체와 적의 사인 곡선 이동은 현재 그레이박스 범위가 아니다.
