...
일반탄은 별도 입력 없이 첫 플레이 틱부터 0.3초마다 오른쪽으로 자동 발사됩니다. 일반 상태에서는 MANA가 초당 5 회복되며, MANA가 20 이상일 때 `Space`를 누르면 일반탄을 멈추고 0.15초마다 최근접 적을 추적하는 스킬탄을 발사합니다. 스킬 중에는 MANA가 초당 30 감소합니다. 일반탄 처치는 MANA 5를 지급하지만 스킬탄 처치는 지급하지 않으며, 추가 회복이나 보상이 들어와도 MANA는 100을 유지합니다.
